...
async def train_models(force: bool = Query(False, description="强制重新训练")):
    ...
    # 使用 initialize_models 方法进行训练
    status = ensemble.initialize_models(force_retrain=force)
    
    training_process = {
        "data_source": "2018俄罗斯世界杯 + 2022卡塔尔世界杯",
        "models": status,
        "success": 'error' not in str(status)
    }
    
    return training_process


# train_neural_network 和 train_random_forest 函数已移除
# 现在使用 ensemble.initialize_models() 进行统一训练
...
async def predict_match(
    ...
    result["predictions"]["bayesian"] = {
        ...
        "result": "主胜" if bayesian.home_win_prob > bayesian.draw_prob and bayesian.home_win_prob > bayesian.away_win_prob else "平局" if bayesian.draw_prob > bayesian.away_win_prob else "客胜",
        "confidence": bayesian.confidence
    }
    
    # 添加神经网络预测
    if ensemble.nn_trained and pred.nn_pred:
        nn_pred = pred.nn_pred
        result["predictions"]["neural_network"] = {
            "model": "神经网络",
            "home_win": nn_pred.home_win_prob,
            "draw": nn_pred.draw_prob,
            "away_win": nn_pred.away_win_prob,
            "result": "主胜" if nn_pred.home_win_prob > nn_pred.draw_prob and nn_pred.home_win_prob > nn_pred.away_win_prob else "平局" if nn_pred.draw_prob > nn_pred.away_win_prob else "客胜"
        }
    
    # 添加随机森林预测
    if ensemble.rf_trained and pred.rf_pred:
        rf_pred = pred.rf_pred
        result["predictions"]["random_forest"] = {
            "model": "随机森林",
            "home_win": rf_pred.home_win_prob,
            "draw": rf_pred.draw_prob,
            "away_win": rf_pred.away_win_prob,
            "result": "主胜" if rf_pred.home_win_prob > rf_pred.draw_prob and rf_pred.home_win_prob > rf_pred.away_win_prob else "平局" if rf_pred.draw_prob > rf_pred.away_win_prob else "客胜"
        }
...
async def predict_match_by_group(group: str, match_idx: int):
    ...
    # 调试：检查模型状态
    import sys
    print(f"[DEBUG] predict_match_by_group: nn_trained={ensemble.nn_trained}, rf_trained={ensemble.rf_trained}", file=sys.stderr)
