...
def create_data_source(
    ...
    **kwargs
    ...
    Args:
        source_type: 数据源类型 (mock, balldontlie, worldcup26)
        config: 配置字典
        **kwargs: 额外参数
    ...
    elif source_type == "worldcup26":
        return WorldCup26Source()
    
    else:
        raise ValueError(
            f"Unknown data source type: {source_type}. "
            f"Available: mock, balldontlie, worldcup26"
        )
...
def get_available_sources() -> Dict[str, str]:
    ...
    return {
        "mock": "模拟数据源（用于开发和测试）",
        "balldontlie": "BALLDONTLIE API（免费但有限制，需要API Key）",
        "worldcup26": "WorldCup26.ir API（完全免费，无需API Key，支持实时数据）"
    }
